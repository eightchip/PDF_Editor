...
#### 方法2: Vercelのビルド環境でのqpdfの制限

**重要**: Vercelのビルド環境では`sudo`権限がないため、`apt-get`で`qpdf`をインストールすることは**できません**。

Vercel環境でのqpdfの利用は以下の制限があります：
- Vercelのビルド環境に既に`qpdf`がインストールされている場合のみ利用可能
- 環境変数`QPDF_PATH`で`qpdf`のパスを指定する必要がある場合があります
- `qpdf`が利用できない場合、パスワード保護なしでPDFをエクスポートできます

**現在の動作**:
- `qpdf`が見つからない場合、エラーメッセージが表示され、保護なしでエクスポートが続行されます
- これは意図的な動作で、アプリケーションは正常に動作し続けます
